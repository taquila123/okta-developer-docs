...
If the Custom URL Domain is configured, you can set a custom domain URL using the **Settings** > **Customization** page in your org. The issuer is then returned in the appropriate OpenID Connect token response. When Custom URL Domain is configured, the default value for new apps is `CUSTOM_URL`. <br>
